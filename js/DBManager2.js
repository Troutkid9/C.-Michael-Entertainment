...
var StateManager = {
  ...
  status: {
    // GameStatus: { status: ST_PAUSED | ST_STOPPED, date: Date }
  ...
  init: function (version, callback) {
    ...
    if (typeof callback !== "undefined") this.initCallback = callback;
    ...
    if (this.db !== null) {
      ...
      if (this.initCallback) this.initCallback(this.gameOptions, this.scores);
    }
    // new Mojo.Depot(this.options, this.onDepotOpened.bind(this), this.onFailure.bind(this));
  },

  clearDB: function () {
    if (this.db !== null) this.db.nuke();
  },
  ...
  setGameOptions: function (gameOptions) {
    ...
    Console.log(
      "setGameOptions: snd = " +
        gameOptions.sound +
        ", back = " +
        gameOptions.background +
        ", cardset= " +
        gameOptions.cardset +
        ", autoFlip=" +
        gameOptions.autoFlip +
        ", autoPlay=" +
        gameOptions.autoPlay +
        ", theme=" +
        gameOptions.theme
    );
  },
  ...
  loadStatus: function () {
    ...
    } else Console.log("Cannot load Status: error accessing the DataBase");
  },
  ...
  loadScores: function (scoresCallback) {
    ...
    } else Console.log("cannot load scores: error accessing the DataBase");
  },
  ...
  saveScores: function () {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save({ key: "scores", value: this.scores }, function (r) {
        if (r) Console.log("saved scores success !!");
        else Console.log("oops: error while saving scores: " + r);
      });
    } else Console.log("cannot save scores: error accessing the DataBase");
  },
  ...
  saveStatus: function (status) {
    ...
    } else Console.log("Cannot save game: error accessing the DataBase");
  },
  ...
  saveGameOptions: function () {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save(
        { key: "gameOptions", value: this.gameOptions },
        function (r) {
          if (r !== null) Console.log("saved gameoptions success !!");
          else Console.log("oops: error while saving gameoptions: " + r);
        }
      );
    } else Console.log("Cannot save gameoptions: error accessing the DataBase");
  },
  ...
  saveState: function (stateObject) {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save({ key: "saveGame", value: stateObject }, function (r) {
        if (r) Console.log("saved success !!");
        else Console.log("oops: error while saving game: " + r);
      });
    } else Console.log("Cannot save game: error accessing the DataBase");
  },
  ...
  removeState: function () {
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.remove("saveGame", function (r) {
        if (r) Console.log("state successfully deleted !!");
        else Console.log("oops: error while removing state game: " + r);
      });
    } else Console.log("Cannot deleteState: error accessing the Database...");
  },
    ...
    if (status !== null) {
      Console.log(
        "Got status: st = " +
          ...
          status.value.date +
          "gamesTotal = " +
          status.value.gamesTotal +
          ", gamesWon=" +
          status.value.gamesWon +
          ", scoreTotal=" +
          status.value.scoreTotal
      );
      this.status = status.value;
    } else {
      Console.log(
        "couldn't get status ! Assume it's the first run, so we generate the object"
      );
  ...
  onLoadGameOptions: function (gameOptions) {
    if (gameOptions !== null) {
      Console.log(
        "Got gameOptions: snd = " +
          gameOptions.value.sound +
          ", back = " +
          gameOptions.value.background +
          ", cardset= " +
          gameOptions.value.cardset +
          ", autoFlip=" +
          gameOptions.value.autoFlip +
          ", autoPlay=" +
          gameOptions.value.autoPlay +
          ", theme=" +
          gameOptions.value.theme
      );
      this.gameOptions = gameOptions.value;
    } else {
      Console.log(
        "couldn't get gameOptions ! Assume it's the first run, so we generate the object with default gameOptions"
      );
  ...
  onLoadScores: function (scores) {
    if (scores !== null) {
      ...
      Console.log(
        "Seems like we have no local scores, let's generate default ones !"
      );
