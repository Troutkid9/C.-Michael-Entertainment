...
var WarpKlondikeMain = (function (game) {
  function bindListeners() {
    $("#playground").css(
      "margin-top",
      parseInt((window.innerHeight - parseInt($("#playground").height())) / 2) +
        "px"
    );

    if (CM.isiOS) $("#sound").hide();
    ...
    $("#showScores").bind(EventsManager.EVENT_UP, function () {
      $(".dialog").center();
      if ($("#scores").hasClass("flipped")) game.togglePauseGame(false);
      else game.togglePauseGame(true);
    ...
    if (!CM.isiOS) {
      $(".icon")
        .bind(EventsManager.EVENT_OVER, function () {
          $(this).addClass("over");
        })
  ...
  function hideSplash() {
    $("#splash").animate({ opacity: 0 }, 500).delay(500).hide();
  }
  ...
  return {
      ...
      if ($themeSheet.length) {
        ...
        $("<link/>")
          .attr({
            rel: "stylesheet",
            type: "text/css",
            href: "css/themes/" + themeName + "/theme.css",
            id: "themeCSS",
          })
          .appendTo("head");
      }
    ...
    setScores: function (scoresArray) {
      if (typeof scoresArray !== "object") return;
