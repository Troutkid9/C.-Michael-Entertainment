...
Card.prototype.init = function () {
  ...
  if (this.visible === true) {
    ...
    if (this.cardtype.indexOf("ace") > -1)
      ...
      this.jQueryElt.css(
        "background-position",
        -(game.spriteXStart + (this.colorNumber - 1) * game.xSpace) +
          "px " +
          "-" +
          game.backsY[this.cardtype] +
          "px"
      );
    }
  } else {
    ...
    this.jQueryElt.css(
      "background-position",
      "-" + game.cardBackX + "px -" + game.backYMisc + "px"
    );
  }
...
Card.prototype.setVisible = function (bool, doNotRotate) {
  ...
  if (this.visible === true) {
    ...
    if (doNotRotate === undefined)
      setTimeout(function () {
        that.jQueryElt.addClass("rotate");
      }, 5);

    setTimeout(function () {
      that.jQueryElt.css(
        "background-position",
        -(game.spriteXStart + (that.colorNumber - 1) * game.xSpace) +
          "px -" +
          game.backsY[that.cardtype] +
          "px"
      );
    }, 250);
...
Card.prototype.onDoubleClick = function (event) {
  ...
  if (
    deckType === DECK_BOTTOM ||
    (deckType === DECK_WASTE && this.deck.getTop() === this)
  ) {
    game.addToAce(this);
  }
};
...
Card.prototype.onOver = function (draggedElt) {
  ...
  if (this.deck !== null) {
    if (this.deck.acceptCard(draggedElt)) this.jQueryElt.css("opacity", 0.5);
  } else {
    var deck = game.bottomDecks[this.deckNum];
    if (deck.acceptCard(draggedElt)) this.jQueryElt.css("opacity", 0.5);
  }
};
...
Card.prototype.onDropping = function (droppedElt) {
  ...
  } // this will put the card back to its place
  else {
...
Card.prototype.toggleDragging = function (bool) {
  if (bool) {
    ...
    game.maxDepth++;
    this.depth(game.maxDepth);
  } else this.jQueryElt.removeClass("dragged");
};
...
Card.prototype.enableSelection = function (bool) {
  if (this.selectable === bool || typeof bool !== "boolean") return;
  ...
  if (this.selectable === true) {
    ...
    this.jQueryElt.bind(
      ...
      jQuery.proxy(function (event) {
        ...
        EventsManager.setDraggingElement(
          this,
          CM.isiOS ? event.originalEvent.touches[0] : event
        );
      }, this)
    );
  } else {
...
Card.prototype.enableDragAndDrop = function (dragBool, dropBool) {
  if (dragBool !== null) this.dragEnabled = dragBool;

  if (dropBool !== null) this.dropEnabled = dropBool;
};
...
Card.prototype.setPosition = function (x, y, x2, y2) {
  ...
  if (typeof x2 !== "undefined") {
    ...
    } else this.jQueryElt.addClass("visible");
  }
