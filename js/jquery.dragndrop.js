...
(function ($) {
  ...
  $.fn.Drops.intersect = function (event) {
    ...
    for (var i = 0; i < dropables.length; i++) {
      if (
        dropables[i].element[0] !== event.data.dragData.target[0] &&
        dropables[i].enabled === true
      ) {
  ...
  $.fn.Drags = function (opts) {
    ...
    return this.each(function () {
      ...
      if (typeof ps.handler == "undefined" || ps.handler == null)
        ...
        handler =
          typeof ps.handler == "string" ? $(ps.handler, this) : ps.handler;
