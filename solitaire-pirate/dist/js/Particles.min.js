function randRange(b, a) {
  return (randomNum = Math.floor(Math.random() * (a - b + 1)) + b);
}
function Fly(a, c) {
  this.x = a;
  this.y = c;
...
Fly.prototype.move = function () {
  ...
  var a = this.x,
    b = this.y;
  if (a > 1024 || a < this.maxX || b > 800 || b < 0) {
    this.hide();
    return;
  }
  if (this.off == true) {
...
var Particles = {
  ...
  init: function (c) {
    this.maxFly = c;
    var d = (this.firstParticle = new Fly(0, 0));
    for (var b = 0, a = c; b < a; b++) {
      d.nextParticle = new Fly(0, 0);
      d = d.nextParticle;
      d.nextParticle = new Fly(0, 0);
      d = d.nextParticle;
      d.nextParticle = new Fly(0, 0);
  ...
  launchParticles: function (c) {
    var b = c.x - 20,
      a = c.x + 96,
      e = c.y - 10,
      d = c.y + 126,
      f = this.firstParticle;
    if (this.interval) {
      clearInterval(this.interval);
      this.interval = null;
    }
