...
Card.prototype.onClick = function (a) {
  game.clickCard(this);
};
Card.prototype.onDoubleClick = function (b) {
  Console.log("doubleclick detected");
  var a = this.deck.getDeckType();
  if (a === DECK_BOTTOM || (a === DECK_WASTE && this.deck.getTop() === this)) {
    Console.log("flick possible !");
    game.addToAce(this);
  }
};
Card.prototype.onOver = function (a) {
  Console.log("" + a + "over " + this);
  if (this.deck !== null) {
    if (this.deck.acceptCard(a)) {
      this.jQueryElt.css("opacity", 0.5);
    }
  } else {
    var b = game.bottomDecks[this.deckNum];
    if (b.acceptCard(a)) {
      this.jQueryElt.css("opacity", 0.5);
    }
  }
};
Card.prototype.onOut = function (a) {
...
Card.prototype.onDropping = function (a) {
  Console.log("dropped ", a, " over ", this);
  ...
  if (b.acceptCard(a)) {
    ...
    Console.log("refused :" + a + " over " + this);
    a.onCancelDragging();
  }
};
  ...
  if (this.selectable === true) {
    this.jQueryElt.bind(
      ...
      jQuery.proxy(function (b) {
        if (game.blockedSelection === true || (b.button && b.button !== 0)) {
          return;
        }
        this.oldCssPosition = {
        ...
        EventsManager.setDraggingElement(
          this,
          CM.isiOS ? b.originalEvent.touches[0] : b
        );
      }, this)
    );
  } else {
...
Card.prototype.setPosition = function (b, f, c, d) {
  var a = this.position,
    e = this;
  if (typeof c !== "undefined") {
    if (b === null) {
      ...
      a.x = c;
      a.y = d;
      this.jQueryElt.css({ top: f + "px", left: b + "px" });
      this.jQueryElt.addClass("visible");
      setTimeout(function () {
        e.jQueryElt.css({ top: d + "px", left: c + "px" });
      }, 25);
    }
  } else {
    if (b != null) {
  ...
  if (this.oldCssPosition == null) {
    this.oldCssPosition = { left: a.x + "px", top: a.y + "px" };
  }
};
...
Card.prototype.depth = function (a) {
  if (typeof a === "undefined" || this.zIndex > a) {
    return this.zIndex;
  } else {
    this.zIndex = a;
