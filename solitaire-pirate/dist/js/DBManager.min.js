...
var StateManager = {
  ...
  scoresCallback: null,
  init: function (a, c) {
    Console.log("initializing StateManager with version " + a);
    this.options = {
      name: "WarpKlondike",
      version: a,
  ...
  setScore: function (b, a) {
    if (a > MAX_SCORES - 1 || a < 0) {
      Console.log(
        "setScore not possible (pos=" + a + ", max=" + (MAX_SCORES - 1) + ")"
      );
      return false;
    } else {
      Console.log(
        "setScore possible !! (pos=" + a + ", max=" + (MAX_SCORES - 1) + ")"
      );
      this.scores[a] = b;
      this.saveScores();
    }
  ...
  saveScores: function () {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save({ key: "scores", value: this.scores }, function (a) {
        if (a) {
          Console.log("saved scores success !!");
        } else {
          Console.log("oops: error while saving scores: " + a);
        }
      });
    } else {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save({ key: "status", value: a }, function (c) {
        if (c !== null) {
          ...
          Console.log("oops: error while saving status: " + c);
        }
      });
    } else {
      ...
      this.db.save(
        ...
        function (b) {
          if (b !== null) {
            ...
            Console.log("oops: error while saving gameoptions: " + b);
          }
        }
      );
    } else {
  ...
  saveState: function (a) {
    ...
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.save({ key: "saveGame", value: a }, function (b) {
        if (b) {
          Console.log("saved success !!");
        } else {
          Console.log("oops: error while saving game: " + b);
        }
      });
    } else {
      Console.log("Cannot save game: error accessing the DataBase");
  ...
  removeState: function () {
    if (this.db !== null && typeof this.db !== "undefined") {
      this.db.remove("saveGame", function (a) {
        if (a) {
          Console.log("state successfully deleted !!");
        } else {
          Console.log("oops: error while removing state game: " + a);
        }
      });
    } else {
  ...
  onFailure: function (a) {
    Console.log("Error opening/creating depot: " + a);
  },
};
