var EventsManager = {
  ...
  initIntersect: function () {
    this.checkIntersectsInterval = setInterval(
      jQuery.proxy(this.checkIntersect, this),
      100
    );
  },
  ...
  init: function () {
    ...
    var mouseUpCallback = jQuery.proxy(function (event) {
      ...
      if (this.dragging) {
        ...
        if (this.checkIntersect()) {
          ...
          if (elt.onCancelDragging) elt.onCancelDragging();
        }
      } else {
        ...
        if (this.dblclickTimeout === null) {
          ...
          this.dblclickTimeout = setTimeout(function () {
            that.dblclickTimeout = null;
          }, DBLCLICK_DELAY);
        } else {
          // second mouse up
          // Console.log('second click !');
          if (elt.onDoubleClick) elt.onDoubleClick();
        }
      }
    ...
    var mouseMoveCallback = jQuery.proxy(function (event) {
      if (!this.draggingElt || !this.draggingElt.dragEnabled) return;

      var draggingElt = this.draggingElt;

      if (!this.dragging) {
        this.dragging = true;
        if (draggingElt.onStartDragging) draggingElt.onStartDragging();
      } else {
        if (draggingElt.onDragging) {
          ...
          draggingElt.onDragging(
            e.pageX - this.gap.x - this.parentOffset.left,
            e.pageY - this.gap.y - this.parentOffset.top
          );
        }
      }
    }, this);
