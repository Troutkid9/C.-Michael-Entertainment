...
Deck.prototype.init = function () {
  ...
  // Add the king card if king deck...
    ...
    this.topCard.setPosition(
      game.xStart + this.deckNum * (game.xSpace + game.packCardSpace),
      game.mainDeckY
    );
    this.topCard.setDeck(this);
  }
  ...
  if (type === DECK_BOTTOM) {
    if (game.touchMode === false) {
      this.topCard.onClick = function () {
        game.cardPos = {
          x: parseInt(this.jQueryElt.css("left")),
          y: parseInt(this.jQueryElt.css("top")),
        };
        game.clickKing(this.deckNum, DECK_BOTTOM);
        return false;
      };
    } else this.topCard.onClick = null;
    ...
    if (game.touchMode === false) {
      jQuery(this.topCard.jQueryElt).bind(EventsManager.EVENT_UP, function () {
        game.cardPos = {
          x: parseInt(jQuery(this).css("left")),
          y: parseInt(jQuery(this).css("top")),
        };
...
Deck.prototype.toggleDragging = function (card, bool) {
  var i = 0;
  (found = false), (max = this.cards.length), (cards = this.cards);
...
Deck.prototype.setPossibleSelections = function () {
  ...
  while (i >= 0 && cards[i].isVisible()) {
    ...
    card = cards[i];
    /* ugly hack */
    if (this === game.wastePack && i != topIndex) card.enableSelection(false);
    else card.enableSelection(true);
...
Deck.prototype.popVisible = function (autoReverse) {
  ...
  if (cards.length && autoReverse == true)
    // do we have elements ?
...
Deck.prototype.popVisibleUpTo = function (card, autoReverse) {
  ...
  if (
    cards.length &&
    autoReverse == true &&
    cards[this.length - 1].isVisible() === false
  )
    // do we have elements ?
    cards[this.length - 1].setVisible(true);
...
Deck.prototype.acceptCard = function (card) {
  ...
  if (deckType == DECK_BOTTOM) {
    ...
    if (this.length === 0) {
      if (card.getColorNumber() == 13) return true;
    } else if (card.getColor() === selfTop.getColor()) return false;
    else return selfTop.getColorNumber() - card.getColorNumber() === 1;
  } else if (card === card.getDeck().getTop()) {
    // only allow to move one card at once over ace decks
    if (deckType == DECK_ACE_HEART) {
      colorNum = selfTop !== null ? selfTop.getColorNumber() : 0;

      if (card.getColorNumber() == 0) return card.getCardType() === "ace0";
      else
        return (
          card.getCardType() === "heart" &&
          card.getColorNumber() - colorNum == 1
        );
    } else if (deckType == DECK_ACE_DIAMOND) {
      colorNum = selfTop !== null ? selfTop.getColorNumber() : 0;

      if (card.getColorNumber() == 0) return card.getCardType() === "ace1";
      else
        return (
          card.getCardType() === "diamond" &&
          card.getColorNumber() - colorNum == 1
        );
    } else if (deckType == DECK_ACE_SPADE) {
      colorNum = selfTop !== null ? selfTop.getColorNumber() : 0;

      if (card.getColorNumber() == 0) return card.getCardType() === "ace2";
      else
        return (
          card.getCardType() === "spade" &&
          card.getColorNumber() - colorNum == 1
        );
    } else if (deckType == DECK_ACE_CLUB) {
      colorNum = selfTop !== null ? selfTop.getColorNumber() : 0;

      if (card.getColorNumber() == 0) return card.getCardType() === "ace3";
      else
        return (
          card.getCardType() === "club" && card.getColorNumber() - colorNum == 1
        );
    }
...
Deck.prototype.getCardAtPos = function (pos) {
  if (pos >= this.cards.length || pos < 0) return null;
  else return this.cards[pos];
};
...
Deck.prototype.setAllCardsVisibility = function (visible) {
  ...
  for (var i = 0, max = this.length; i < max; i++) {
    ...
    if (visible) cards[i].jQueryElt.addClass("visible");
    else cards[i].jQueryElt.removeClass("visible");
  }
};
