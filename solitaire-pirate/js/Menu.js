...
function Menu(DOMSelector, menuParams, selectedMenu) {
  ...
  this.onSelect =
    menuParams.onSelect !== "undefined" ? menuParams.onSelect : null;
  this.onDestroy =
    menuParams.onDestroy !== "undefined" ? menuParams.onDestroy : null;
...
Menu.prototype.init = function () {
  ...
  if (CM.DEBUG) Mojo.Log.info("[Menu Class] Initializing Menu...");

  // Generate HTML list
  this.optionsList = new Element("ul");
  while (i < this.length) {
    var li = new Element("li", { class: "menu" });
    li.update("<a>" + this.entries[i].title + "</a>")
      .hide()
      .insert({
        bottom: '<div class="shadow">' + this.entries[i].title + "</div>",
      });
...
Menu.prototype.showEntry = function (i, bool) {
  if (i < this.length) this.entries[i].disabled = !bool;
};
...
Menu.prototype.toggle = function (effect) {
  if (this.shown === true) {
    ...
    this.optionsList.select("li").each(function (element, i) {
      if (
        typeof that.entries[i].disabled === "undefined" ||
        that.entries[i].disabled === false
      ) {
